...
export default async function AnalyticsPage() {
  ...
  const totalRevenue = dbData.stats.totalRevenue;
  ...
  const conversionRatePercent = dbData.stats.totalCustomers > 0
    ? Math.min((dbData.stats.totalOrders / dbData.stats.totalCustomers) * 100, 16.5).toFixed(1) + "%"
    : "0.0%";
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-4 border-b border-zinc-805/80 w-full">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] bg-emerald-950 text-emerald-305 text-emerald-400 border border-emerald-900/60 px-2.5 py-0.5 rounded-full font-mono font-bold uppercase tracking-wider">
              Results
            </span>
          </div>
      ...
      <div className="bg-zinc-950/45 border border-zinc-900 p-5 rounded-2xl">
        ...
        <div className="space-y-3 font-mono text-xs text-zinc-450 text-zinc-400">
          {/* Dispatch Bar */}
